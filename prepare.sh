...
if [ "$(uname -s)" = "Darwin" ]; then
  ...
  if command -v brew &> /dev/null; then
    ...
    ubuntu | linuxmint | debian | pop)
        apt-get update
        apt-get -y install clang llvm texinfo bison flex gettext libgmp3-dev libmpfr-dev libmpc-dev libusb-1.0-0-dev libreadline-dev libcurl4 \
    ...
    ;;
    rhel | fedora)
         dnf -y install @development-tools clang llvm gcc gcc-c++ g++ wget git autoconf automake make cmake pkgconf \
    ...
    gentoo)
        emerge --noreplace net-misc/wget dev-vcs/git sys-apps/fakeroot \
                                        ...
                                        sys-devel/clang sys-devel/llvm dev-libs/mpc dev-libs/libusb dev-build/ninja dev-build/meson
    ;;
    arch | manjaro | endeavouros | cachyos)
        pacman -Sy gcc clang llvm make cmake patch git texinfo flex bison gettext wget gsl gmp mpfr libmpc libusb readline libarchive gpgme bash openssl libtool boost meson ninja
    ;;
    opensuse*)
      zypper install -y gcc gcc-c++ clang llvm binutils patch make cmake bison flex gpgme libgpgme-devel libarchive-devel openssl libopenssl-devel ncurses ncurses-devel gmp-devel mpfr-devel mpc-devel \
